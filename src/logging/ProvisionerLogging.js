/* @flow */
import type { AdjustmentContext, AdjustmentData } from '../flow/FlowTypes';
import log from '../logging/log';

export default class ConfigLogging {
  static isAdjustmentRequiredLog(adjustmentContext: AdjustmentContext, adjustmentData: AdjustmentData) {
    let logMessage = typeof adjustmentContext.IndexName === 'undefined'
      ? adjustmentContext.TableName
      : adjustmentContext.TableName + '.' + adjustmentContext.IndexName;
    logMessage +=
      ' is consuming ' +
      ...
      ' (' +
      adjustmentContext.UtilisationPercent +
      '%) ' +
      adjustmentContext.CapacityType +
      ' capacity units';
    ...
    if (
      ...
      adjustmentData.isAboveThreshold &&
      !adjustmentData.isAboveMax
    ) {
      logMessage +=
        ' and is above maximum threshold of ' +
    ...
    if (adjustmentContext.CapacityConfig.Min != null && adjustmentData.isBelowMin) {
      logMessage += ' and is below the min allowed ' + adjustmentContext.CapacityConfig.Min + ' units';
    }

    if (
      adjustmentContext.CapacityAdjustmentConfig != null &&
      adjustmentContext.CapacityAdjustmentConfig.When.UtilisationIsBelowPercent != null &&
      adjustmentData.isBelowThreshold &&
      !adjustmentData.isBelowMin
    ) {
      logMessage +=
        ' and is below minimum threshold of ' +
    ...
    if (
      ...
      logMessage +=
        ' and throttled events per minute is above ' +
        adjustmentContext.CapacityAdjustmentConfig.When.ThrottledEventsPerMinuteIsAbove +
        ' events';
    }

    if (adjustmentData.isAdjustmentWanted) {
      logMessage += adjustmentContext.AdjustmentType === 'increment'
        ? ' so an increment is WANTED'
        : ' so a decrement is WANTED';
      ...
      logMessage += adjustmentContext.AdjustmentType === 'increment'
        ? ' so an increment is not required'
        : ' so a decrement is not required';
    }
