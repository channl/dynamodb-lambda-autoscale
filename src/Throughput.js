class Throughput {
  ...
  static getPercentAdjustedReadCapacityUnits(data, adjustmentPercent, adjustmentUnits, max, min) {
    let units = Math.round(data.ProvisionedThroughput.ReadCapacityUnits * (adjustmentPercent / 100));
  ...
  static getPercentAdjustedWriteCapacityUnits(data, adjustmentPercent, adjustmentUnits, max, min) {
    let units = Math.round(data.ProvisionedThroughput.WriteCapacityUnits * (adjustmentPercent / 100));
    units = Math.max(units, adjustmentUnits);
